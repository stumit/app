...
export default defineComponent({
  ...
  setup(){
    ...
    onMounted(() => [
      // 当组件加载完毕时,向vuex发送获取数据请求
      store.dispatch('getBannerList'),
      
    ]);
    // 通过计算属性获取vuex中bannerList的数据到此组件
        ...
        nextTick(()=>{
          new Swiper(".swiper", {
            ...
            autoplay: {
              // 自动切换时间
              delay: 3000,
              // 为true则不会循环播放,但设置了loop则无效
              stopOnLastSlide: false,
              // 为true则在操作了后,停止autoplay自动切换
              disableOnInteraction: false,
              // 为true,鼠标悬浮则停止切换,离开恢复自动切换
